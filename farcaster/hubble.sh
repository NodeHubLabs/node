...
validate_and_store() {
    ...
    if [[ $lower_response == *'"result":"'$lower_expected_chain_id'"'* ]]; then
        echo "$env_key=$rpc_url" >> .env
    else
        echo "Validating $rpc_url for $rpc_name..."
        echo "!!! Invalid !!!"
        echo "$rpc_name Ethereum RPC URL or chainID is not $expected_chain_id. Please check your environment variable."
...
write_env_file() {
    ...
    if ! key_exists "ETH_MAINNET_RPC_URL"; then
        validate_and_store "Ethereum Mainnet" "0x1" "ETH_MAINNET_RPC_URL" "$ETH_MAINNET_RPC_URL"
    fi

    if ! key_exists "OPTIMISM_L2_RPC_URL"; then
        echo "OPTIMISM_L2_RPC_URL=$OPTIMISM_L2_RPC_URL"
        validate_and_store "Optimism L2 Mainnet" "0xa" "OPTIMISM_L2_RPC_URL" "$OPTIMISM_L2_RPC_URL"
    fi

    if ! key_exists "HUB_OPERATOR_FID"; then
        store_operator_fid_env "$HUB_OPERATOR_FID"
    fi
